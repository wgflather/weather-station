...
class WeatherReportRepositoryTest {
    ...
    @Container
    @ServiceConnection
    static PostgreSQLContainer container = new PostgreSQLContainer(DockerImageName.parse("postgres:15.1"));
    ...
    void saveNewRecordWithOkDataQuality() throws Exception {
        WeatherRecordCreatedDto createdDto = WeatherRecordCreatedDto.builder()
                .temperature(10)
                .pressure(1000)
                .measuredAt(Instant.now().truncatedTo(ChronoUnit.SECONDS))
                .build();

        mockMvc.perform(post(WeatherController.BASE_PATH)
    ...
    void testDataQualitySpikeCondition() throws Exception{
        ...
        WeatherRecordCreatedDto createdFirstRecord = WeatherRecordCreatedDto.builder()
                .temperature(10)
                .pressure(1000)
                .measuredAt(now)
                .build();

        WeatherRecordCreatedDto spikedDto = WeatherRecordCreatedDto.builder()
                .temperature(25)
                .pressure(1000)
        ...
        List<WeatherRecord> records = repository.findAll()
                .stream()
                .sorted(Comparator.comparing(WeatherRecord::getMeasuredAt)).toList();

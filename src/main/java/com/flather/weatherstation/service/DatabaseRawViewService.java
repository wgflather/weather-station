...
public class DatabaseRawViewService {
  ...
  public Page<WeatherRecord> getRecordsByDate(LocalDate date, Pageable pageable) {
    Instant startDate =
        date.atStartOfDay(configurationCache.getLocationContext().zoneId()).toInstant();
    Instant endDate =
        date.atStartOfDay(configurationCache.getLocationContext().zoneId()).plusDays(1).toInstant();
  ...
  public Page<WeatherRecord> getRecordBetween(LocalDate start, LocalDate end, Pageable pageable) {
    Instant startDate =
        start.atStartOfDay(configurationCache.getLocationContext().zoneId()).toInstant();
    Instant endDate =
        end.atStartOfDay(configurationCache.getLocationContext().zoneId()).plusDays(1).toInstant();
    return repository.getRecordsBetween(startDate, endDate, pageable);
  }

  public Page<WeatherRecord> getRecords(
      boolean all,
      DataQuality quality,
      Pageable pageable,
      Metric metric,
  ...
  private Instant toEndInstant(LocalDate date) {
    return date.atStartOfDay(configurationCache.getLocationContext().zoneId())
        .plusDays(1)
        .toInstant();
  }

  private Page<WeatherRecord> findByMetricAndQualityBetween(
      Metric metric, DataQuality quality, Instant start, Instant end, Pageable pageable) {
    return switch (metric) {
      ...
      case WIND_DIRECTION ->
          repository.findByWindDirectionQualityBetween(quality, start, end, pageable);
  ...
  public Page<WeatherRecord> findByMetricAndQuality(
      Metric metric, DataQuality quality, Pageable pageable) {
    ...
    return switch (metric) {
      ...
      case WIND_DIRECTION -> repository.findByWindDirectionDataQuality(quality, pageable);

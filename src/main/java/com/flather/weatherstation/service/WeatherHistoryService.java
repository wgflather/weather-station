...
public class WeatherHistoryService {
  ...
  private List<DataPoint> findHourlyDataPoints(Metric metric, Instant from, Instant to) {
    return switch (metric) {
      ...
      default -> throw new IllegalArgumentException("Unsupported metric for history chart: " + metric);
    };
  }

  private List<HourlyChartAvgDto> toChartDtos(List<DataPoint> points) {
    ZoneId zoneId = configurationCache.getLocationContext().zoneId();
    return points.stream()
        .map(p -> new HourlyChartAvgDto(p.hour().atZone(zoneId), p.value()))
        .toList();
  }
}

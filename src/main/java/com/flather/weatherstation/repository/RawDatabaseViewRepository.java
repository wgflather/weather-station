...
public interface RawDatabaseViewRepository
    ...
    """)
    Page<WeatherRecord> findByQualityBetween(@Param("quality") DataQuality quality,
                                             @Param("start") Instant start,
                                             @Param("end") Instant end,
                                             Pageable pageable);

    @Query("SELECT w FROM WeatherRecord w WHERE w.temperatureDataQuality = :quality AND w.measuredAt >= :start AND w.measuredAt < :end")
    Page<WeatherRecord> findByTemperatureQualityBetween(@Param("quality") DataQuality quality,
                                                        @Param("start") Instant start,
                                                        @Param("end") Instant end,
                                                        Pageable pageable);

    @Query("SELECT w FROM WeatherRecord w WHERE w.pressureDataQuality = :quality AND w.measuredAt >= :start AND w.measuredAt < :end")
    Page<WeatherRecord> findByPressureQualityBetween(@Param("quality") DataQuality quality,
                                                     @Param("start") Instant start,
                                                     @Param("end") Instant end,
                                                     Pageable pageable);

    @Query("SELECT w FROM WeatherRecord w WHERE w.humidityDataQuality = :quality AND w.measuredAt >= :start AND w.measuredAt < :end")
    Page<WeatherRecord> findByHumidityQualityBetween(@Param("quality") DataQuality quality,
                                                     @Param("start") Instant start,
                                                     @Param("end") Instant end,
                                                     Pageable pageable);

    @Query("SELECT w FROM WeatherRecord w WHERE w.surfaceWetnessDataQuality = :quality AND w.measuredAt >= :start AND w.measuredAt < :end")
    Page<WeatherRecord> findBySurfaceWetnessQualityBetween(@Param("quality") DataQuality quality,
                                                           @Param("start") Instant start,
                                                           @Param("end") Instant end,
                                                           Pageable pageable);
    ...
    @Query(value = """
            ...
            ORDER BY 1 DESC
            """, nativeQuery = true)
    List<String> findDistinctMeasuredDatesBetween(@Param("zone") String zone,

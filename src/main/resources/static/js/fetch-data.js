...
/**
 * Paints the "now" marker as a miniature sun for the given altitude.
 *
 ...
 * layer's RGB toward black, so a set sun dims to a dark warm ember. (Fading
 * the element's opacity instead would blend it into the navy card and read
 * grey, losing the warmth.)
 */
function applySunMarkerAppearance(el, altitude) {
    ...
    const centre = dim(lerpTriplet(SUN_IVORY, rgb, coreMix * 0.45));
    const core   = dim(lerpTriplet(SUN_IVORY, rgb, coreMix));
    const halo   = dim(lerpTriplet(SUN_IVORY, rgb, haloMix));
    const bloom  = dim(lerpTriplet(SUN_IVORY, rgb, bloomMix));
    ...
    // on a hard circle.
    el.style.setProperty('--now-centre',    rgbString(centre));
    el.style.setProperty('--now-core',      rgbString(core));
    el.style.setProperty('--now-halo',      rgbString(halo));
    el.style.setProperty('--now-edge',      rgbaString(halo, 0.30 + 0.68 * sphere));
    el.style.setProperty('--now-edge-stop', `${(64 + 28 * sphere).toFixed(0)}%`);
    el.style.setProperty('--now-highlight', rgbaString([255, 255, 255], 0.03 + 0.42 * sphere));
    ...
    el.style.setProperty('--now-glow-inner-color', rgbaString(halo,  (0.42 + 0.20 * warmth) * glowLum));
    el.style.setProperty('--now-glow-mid-color',   rgbaString(bloom, (0.20 + 0.16 * warmth) * glowLum));
    el.style.setProperty('--now-glow-outer-color', rgbaString(bloom, (0.07 + 0.10 * warmth) * glowLum));

    // Spikes inherit the inner-halo colour, so they warm with the atmosphere.
    el.style.setProperty('--now-spike',         rgbString(halo));
    el.style.setProperty('--now-spike-opacity', (0.10 + 0.26 * sphere).toFixed(2));
}
...
// wavelengths scatter out, warming to near-white overhead. Drives the sun
// card's "now" marker so it reads as a tiny rendering of the real sun rather
// than a generic UI dot.
...
function sunDiskColorAt(altDeg) {
    const first = SUN_DISK_ANCHORS[0];
    const last  = SUN_DISK_ANCHORS[SUN_DISK_ANCHORS.length - 1];
    if (altDeg <= first.alt) return first.rgb;
    if (altDeg >= last.alt)  return last.rgb;
